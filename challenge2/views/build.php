<div class="col-sm-12"  style="border:3px gray solid; margin:3px;padding:3px;" >
        <form action"" method="post">
        <h3>活動名稱 :<input type="text" name="employee"></h3>
        <h3>活動人數 :<input type="text" name="employee"></h3>
        是否攜伴:
        <input type="radio" name="partner" value="no"required="required"> 否
        <input type="radio" name="partner" value="yes"required="required"> 是
        <h3>報名日期:<input type="text" name="employee">~ 截止日期時間數:<input type="text" name="employee"></h3>~
       
        參加方式:
        <input type="radio" name="accenmethod" value="accending"required="required"> 報名參加
        <input type="radio" name="accenmethod" value="free"required="required"> 自由參加
        <br>
        <h3><input type="submit" name="submit" value="submit" style="position:center;"></h3>
        </form>
</div>




