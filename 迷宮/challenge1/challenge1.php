 ...
 $num2=0;
    while(isset($origin[$i][$num2])){//找出二維陣列
          if($origin[$i][$num2]==1 && $check[$i][$num2]==null ){//如果陣列是1且在check陣列裡記錄不還沒有紀錄為一個區塊 
              ...
              }if($check[$i-1][$num2]!=null){//上方
               $check[$i][$num2]=$check[$i-1][$num2];

              }if($check[$i][$num2-1]!=null){//左方
               $check[$i][$num2]=$check[$i][$num2-1];
               
              }if($check[$i][$num2+1]!=null){//右方
               $check[$i][$num2]=$check[$i][$num2+1];
               
              }if($check[$i+1][$num2]!=null){//下方
              ...
              $color=array("#003366","#660000","#ff8000","#ff0080","#00ffff","#000000","#82007D","#99FFCC");
           echo "<font color=$color[$block]>".$check[$i][$num2]." "."</font>";
          }else{
           echo "0 ";
          }
      $num2+=1;
...
//echo "//從最後面找回去 如果相鄰數字比自己小 代表自己屬於其他更早的區塊<br>"; 
   ...
   while($a>=0){//從最後面找回去 如果相鄰數字比自己小 代表自己屬於其他更早的區塊
         while($b>=0){
          if($check[$a][$b]!=null){
              if(isset($check[$a-1][$b]) && $check[$a][$b]!=$check[$a-1][$b]){//上方較小就合併到裡面 較大就合併她
                  if($check[$a][$b]>$check[$a-1][$b]){
                      $check[$a][$b]=$check[$a-1][$b];
                  }
                  if($check[$a][$b]<$check[$a-1][$b]){
                    $check[$a-1][$b]=$check[$a][$b];
                  }
              }
             
              if(isset($check[$a][$b-1]) && $check[$a][$b]!=$check[$a][$b-1] ){//左方較小就合併到裡面 較大就合併她
                  if($check[$a][$b]>$check[$a][$b-1]){
              ...
              if(isset($check[$a][$b+1]) && $check[$a][$b]!=$check[$a][$b+1]){//右方較小就合併到裡面 較大就合併她
                  if($check[$a][$b]>$check[$a][$b+1]){
                      $check[$a][$b]=$check[$a][$b+1];
                   }
                  if($check[$a][$b]<$check[$a][$b+1]){
              ...
              if(isset($check[$a+1][$b]) && $check[$a][$b]!=$check[$a+1][$b]  ){//下方較小就合併到裡面 較大就合併她
                  if($check[$a][$b]>$check[$a+1][$b]){
                      $check[$a][$b]=$check[$a+1][$b];
                  }
                  if($check[$a][$b]<$check[$a+1][$b]){
    ...
    $num2=0;
         while($num2<count($check)){
          if(isset($check[$num][$num2]) && isset($recoed2[$num][$num2])){
              echo "<font color=red>".$check[$num][$num2]." "."</font>";
           }
         else
         echo "0 ";
